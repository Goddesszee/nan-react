...
function CheckoutPage({
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6 pb-28 lg:pb-8">
      ...
      <Card padding="md" className="mb-4">
        <p className="text-xs font-bold text-[#6b6580] uppercase tracking-wider mb-3">Order summary</p>
        <div className="space-y-2 mb-3">
          {cart.map((item: CartItem) => (
            <div key={item.product.id} className="flex justify-between text-sm">
              <span className="text-[#334155] truncate pr-2">{item.product.name} ×{item.quantity}</span>
              <span className="font-semibold text-[#122d45] tabular-nums flex-shrink-0">
                {formatUSDC(item.product.price * item.quantity)} USDC
              </span>
            </div>
          ))}
      ...
      <Card padding="md" className="mb-5">
        <p className="text-xs font-bold text-[#6b6580] uppercase tracking-wider mb-2">Payment method</p>
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-[#dbeafe] flex items-center justify-center">
            <ShoppingBag size={15} className="text-[#1a6fd4]" />
          </div>
          <div>
            <p className="text-sm font-semibold text-[#122d45]">USDC Wallet</p>
            <p className="text-xs text-[#6b6580]">Arc Testnet · Gas-free</p>
          </div>
      ...
      >
        {processing ? 'Processing...' : `Confirm · ${formatUSDC(total)} USDC`}
      </Button>
      <p className="text-center text-xs text-[#8a849c] mt-3">
