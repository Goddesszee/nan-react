...
// ═══════════════════════════════════════════
// PAYMENT REQUEST — clean, no on-chain calls
// ═══════════════════════════════════════════
let _nprTok='USDC', _nprActiveId=null;
function nprSetTok(t,el){ _nprTok=t; document.querySelectorAll('#nprBtnUsdc,#nprBtnEurc').forEach(b=>b.classList.remove('active')); if(el)el.classList.add('active'); }
function _nprLoad(){ if(!userAddr)return []; try{ return JSON.parse(localStorage.getItem('nan_npr_'+userAddr)||'[]'); }catch{ return []; } }
function _nprSave(list){ if(!userAddr)return; localStorage.setItem('nan_npr_'+userAddr,JSON.stringify(list)); }
function _nprLink(pr){ const b=window.location.origin+window.location.pathname; return b+'?pay='+pr.id+'&to='+encodeURIComponent(pr.to)+'&amt='+(pr.amount||'')+'&tok='+pr.token+'&lbl='+encodeURIComponent(pr.label||'')+'&nte='+encodeURIComponent(pr.note||''); }
function nprRender(){
  var list=_nprLoad();
  var el=document.getElementById('nprList'); if(!el)return;
  var t=list.length, paid=list.filter(function(p){return p.status==='paid';}).length, pend=list.filter(function(p){return p.status!=='paid';}).length;
  var e1=document.getElementById('nprStatTotal'),e2=document.getElementById('nprStatPaid'),e3=document.getElementById('nprStatPending');
  if(e1)e1.textContent=t; if(e2)e2.textContent=paid; if(e3)e3.textContent=pend;
  if(!t){ el.innerHTML='<div style="text-align:center;padding:40px 16px;"><div style="font-size:2.5rem;margin-bottom:10px;">🧾</div><div style="font-size:.9rem;font-weight:700;color:var(--text);margin-bottom:6px;">No requests yet</div><div style="font-size:.78rem;color:var(--text3);margin-bottom:16px;">Create one and share instantly</div><button onclick="goPage('payreq-new')" style="background:#6d28d9;border:none;border-radius:10px;color:#fff;font-family:'Inter',sans-serif;font-weight:700;font-size:.82rem;padding:10px 20px;cursor:pointer;">+ Create Request</button></div>'; return; }
  el.innerHTML=list.map(function(pr){
    var sc=pr.status==='paid'?'#34d399':'#fbbf24'; var sl=pr.status==='paid'?'✓ Paid':'⏳ Pending';
    var at=pr.amount?parseFloat(pr.amount).toFixed(2)+' '+pr.token:'Open · '+pr.token;
    return '<div onclick="nprView(''+pr.id+'')" style="display:flex;align-items:center;gap:12px;padding:13px 14px;background:var(--surface);border:1px solid var(--border);border-radius:12px;cursor:pointer;margin-bottom:8px;" onmouseover="this.style.background='rgba(109,40,217,.06)'" onmouseout="this.style.background='var(--surface)'"><div style="width:40px;height:40px;border-radius:10px;background:rgba(109,40,217,.1);display:flex;align-items:center;justify-content:center;font-size:1.1rem;flex-shrink:0;">🧾</div><div style="flex:1;min-width:0;"><div style="font-size:.88rem;font-weight:600;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">'+pr.label+'</div><div style="font-size:.72rem;color:var(--text3);">'+new Date(pr.createdAt).toLocaleDateString()+'</div></div><div style="text-align:right;flex-shrink:0;"><div style="font-size:.88rem;font-weight:700;color:var(--text);">'+at+'</div><div style="font-size:.68rem;font-weight:600;color:'+sc+';">'+sl+'</div></div></div>';
  }).join('');
}
function nprCreate(){
  var lbl=(document.getElementById('nprLbl').value||'').trim();
  var amt=parseFloat(document.getElementById('nprAmt').value)||null;
  var nte=(document.getElementById('nprNte').value||'').trim();
  if(!lbl){ toast('Enter a description','error'); return; }
  if(!userAddr){ toast('Connect wallet first','error'); return; }
  var id='npr_'+Date.now().toString(36)+Math.random().toString(36).slice(2,6);
  var pr={id:id, to:userAddr, token:_nprTok, amount:amt, label:lbl, note:nte, status:'pending', createdAt:Date.now()};
  var list=_nprLoad(); list.unshift(pr); _nprSave(list);
  toast('✓ Request created!','success',3000);
  document.getElementById('nprLbl').value=''; document.getElementById('nprAmt').value=''; document.getElementById('nprNte').value='';
  goPage('payreq'); nprRender();
}
function nprView(id){
  var list=_nprLoad(); var pr=list.find(function(p){return p.id===id;});
  if(!pr)return; _nprActiveId=id;
  var link=_nprLink(pr);
  document.getElementById('nprViewLbl').textContent=pr.label;
  ...
  document.getElementById('nprViewDot').style.background=pr.status==='paid'?'#34d399':'#fbbf24';
  document.getElementById('nprViewTo').textContent=short(pr.to);
  document.getElementById('nprViewTok').textContent=pr.token;
...
function nprCopy(){
  var list=_nprLoad(); var pr=list.find(function(p){return p.id===_nprActiveId;}); if(!pr)return;
  var link=_nprLink(pr);
  navigator.clipboard.writeText(link).then(function(){ toast('✓ Link copied!','success',2500); }).catch(function(){ toast('Link: '+link,'info',6000); });
}
function nprShare(platform){
  var list=_nprLoad(); var pr=list.find(function(p){return p.id===_nprActiveId;}); if(!pr)return;
  var link=_nprLink(pr);
  var amt=pr.amount?pr.amount+' '+pr.token:pr.token;
  var text='Pay '+amt+' for "'+pr.label+'" via NAN Wallet: '+link;
  var urls={
    whatsapp:'https://wa.me/?text='+encodeURIComponent(text),
    twitter:'https://twitter.com/intent/tweet?text='+encodeURIComponent(text),
    telegram:'https://t.me/share/url?url='+encodeURIComponent(link)+'&text='+encodeURIComponent('Pay '+pr.label+' via NAN'),
    email:'mailto:?subject='+encodeURIComponent('Payment Request: '+pr.label)+'&body='+encodeURIComponent(text)
  };
  if(urls[platform]) window.open(urls[platform],'_blank');
}
function nprDelete(){
  if(!_nprActiveId)return;
  var list=_nprLoad(); _nprSave(list.filter(function(p){return p.id!==_nprActiveId;}));
  _nprActiveId=null; toast('Deleted','info',2000); goPage('payreq'); nprRender();
}
