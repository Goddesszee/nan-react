...
const CIRBTC_ADDR    = '0xf0C4a4CE82A5746AbAAd9425360Ab04fbBA432BF'; // cirBTC on Arc Testnet
const CIRBTC_DECIMALS = 8; // cirBTC uses 8 decimals (BTC standard)
...
let regType='x', swapFlipped=false, sendToken='USDC', cirbtcBal=0;
let onChainStaked=0;
...
async function submitAirtime(){
  ...
  try{
    ...
    statusEl.innerHTML = '<span style="color:var(--text);">Payment sent ('+usdcAmount.toFixed(4)+' USDC) · Delivering airtime…</span>';
...
async function submitData(){
  ...
  try{
    const { txHash, usdcAmount } = await payNgnInUsdc(ngnAmount, btn);
    statusEl.innerHTML = '<span style="color:var(--text);">Payment sent ('+usdcAmount.toFixed(4)+' USDC) · Delivering data…</span>';
...
async function doCredit(){
  if(!userAddr){ toast('Connect your wallet first', 'error', 3000); return; }
  openBillModal('Credit Score', `<div style="text-align:center;padding:40px 0;"><span class="spinner"></span><div style="font-size:.85rem;color:var(--text3);margin-top:12px;">Computing on-chain reputation…</div></div>`);
...
function updateAjoPreview(){
  const previewEl = document.getElementById('ajoPreview');
  if(!previewEl) return;
...
async function submitAjoCreate(){
  ...
  try{
    ...
    if(isCircleWallet){
      statusEl.innerHTML='<span style="color:var(--text);">Sending to chain…</span>';
      const d=await ajoApi('createGroup',{contributionAmount:contribution,maxMembers,roundLength,label});
      if(!d.success) throw new Error(d.error||'Failed');
      statusEl.innerHTML='<span style="color:var(--text);">Confirmed ✓</span>';
      ...
      statusEl.innerHTML='<span style="color:var(--text);">Confirming on-chain…</span>';
      const receipt=await tx.wait(1);
...
async function submitAjoJoin(){
  ...
  try{
    if(isCircleWallet){
      ...
      statusEl.innerHTML='<span style="color:var(--text);">Confirming…</span>';
      await tx.wait(1);
    }
...
async function showAjoGroup(groupId){
  ...
  try{
    ...
    const memberRows=members.map((m,i)=>{
      ...
      else if(isPastRound) rightTag=`<span style="font-size:.75rem;color:var(--text);">Received ✓</span>`;
      return `<div style="display:flex;align-items:center;gap:10px;padding:9px 0;border-bottom:0.5px solid rgba(255,255,255,.05);">
        <div style="width:20px;font-size:.7rem;color:var(--text);text-align:right;flex-shrink:0;">${i+1}</div>
    ...
    if(isOpen){
      if(isCreator&&Number(g.memberCount)>=Number(g.maxMembers)){
        actionHtml=`<button onclick="submitAjoStart(${groupId})" id="ajoActionBtn" style="width:100%;background:#2563EB;border:none;border-radius:14px;color:var(--text);padding:15px;font-size:.95rem;font-weight:700;cursor:pointer;margin-top:4px;">Everyone's here — Start group</button>`;
      } else if(isCreator){
      ...
      if(!already){
        ...
        actionHtml=`<button onclick="submitAjoClaim(${groupId})" id="ajoCreatorPayBtn" style="width:100%;background:#2563EB;border:none;border-radius:14px;color:var(--text);padding:15px;font-size:.95rem;font-weight:700;cursor:pointer;margin-top:4px;">Release payout to ${recipientLabel}</button>`;
      } else if(isCreator&&!everyonePaid){
...
async function submitAjoContribute(groupId){
  ...
  try{
    ...
    if(isCircleWallet){
      statusEl.innerHTML='<span style="color:var(--text);">Approving & contributing…</span>';
...
async function submitAjoClaim(groupId){
  ...
  try{
    if(isCircleWallet){
      if(statusEl) statusEl.innerHTML='<span style="color:var(--text);">Sending to chain…</span>';
      const d=await ajoApi('claimRoundPayout',{groupId});
      if(!d.success) throw new Error(d.error||'Failed');
      if(statusEl) statusEl.innerHTML='<span style="color:var(--text);">Confirmed ✓</span>';
    } else {
      ...
      if(statusEl) statusEl.innerHTML='<span style="color:var(--text);">Confirming…</span>';
      await tx.wait(1);
    }
...
async function submitNepa(){
  ...
  try{
    const { txHash, usdcAmount } = await payNgnInUsdc(ngnAmount, btn);
    statusEl.innerHTML = '<span style="color:var(--text);">Payment sent ('+usdcAmount.toFixed(4)+' USDC) · Processing…</span>';
...
function downloadReceipt(){
  ...
  tg.addColorStop(0,'#e9d5ff');tg.addColorStop(0.5,'#fff');tg.addColorStop(1,'#60A5FA');
...
function _setSendTabOrig(tab){
  ...
  if(tab==='receive'&&userAddr){
    ...
    if(box){
      ...
      catch(e){box.innerHTML='<p style="padding:10px;font-size:.7rem;color:var(--text)">QR unavailable</p>';}
    }
...
function flipSwap(explicitMode){
  // Cycle: 0=USDC→EURC, 1=EURC→USDC, 2=USDC→cirBTC, 3=cirBTC→USDC
  if(typeof window._swapMode==='undefined') window._swapMode=0;
  window._swapMode = (typeof explicitMode==='number') ? explicitMode : (window._swapMode+1)%4;
  swapFlipped=(window._swapMode===1||window._swapMode===3);
  _quoteCache={};
  ...
  const CIRBTC_LOGO='data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><circle cx="16" cy="16" r="16" fill="%23F7931B"/><path d="M23.19 13.54c.32-2.1-1.29-3.24-3.48-4l.71-2.85-1.73-.43-.69 2.77-1.39-.34.69-2.77-1.73-.43-.71 2.84-1.11-.27-2.39-.6-.71 2.84 1.38.35-.97 3.9 1.38.34.01.01-.01-.01-1.07 4.31c-.14.17-.32.43-.72.34l-1.38-.34-.95 3.79 2.26.56-.71 2.85 1.49.37-.71 2.84 1.39.35.71-2.84 1.38.34-.71 2.84 1.73.43.71-2.84c2.25.56 4.17.39 5.52-.88 1.1-1.03 1.43-2.48.9-3.84 1.31-.5 2.25-1.6 2.52-3.24zM16.85 19.66c-.39 1.57-4.07.75-5.22.47l.93-3.73c1.16.29 4.87.83 4.29 3.26zm.41-6.16c-.36 1.43-3.54.7-4.53.47l.84-3.39c1 .25 4.03.71 3.69 2.92z" fill="%23fff"/></svg>';
  const modes=[
    {from:'USDC',to:'EURC',fromLogo:USDC_LOGO,toLogo:EURC_LOGO,fromBal:usdcBal,toBal:eurcBal,pair:'usdc-eurc'},
    {from:'EURC',to:'USDC',fromLogo:EURC_LOGO,toLogo:USDC_LOGO,fromBal:eurcBal,toBal:usdcBal,pair:'usdc-eurc'},
    {from:'USDC',to:'cirBTC',fromLogo:USDC_LOGO,toLogo:CIRBTC_LOGO,fromBal:usdcBal,toBal:cirbtcBal,pair:'cirbtc'},
    {from:'cirBTC',to:'USDC',fromLogo:CIRBTC_LOGO,toLogo:USDC_LOGO,fromBal:cirbtcBal,toBal:usdcBal,pair:'cirbtc'}
  ];
  const m=modes[window._swapMode||0];
  applySwapModeObj(m);
}

// Shared logic used by both flipSwap() (cycling) and pickSwapToToken()
// (explicit dropdown selection) to avoid duplicating the token/logo data.
function applySwapModeObj(m){
  const fromLbl=document.getElementById('fromTokenLabel');
  const toLbl=document.getElementById('toTokenLabel');
  const fromLogo=document.getElementById('fromTokenLogo');
  const toLogo=document.getElementById('toTokenLogo');
  if(fromLbl) fromLbl.textContent=m.from;
  if(toLbl) toLbl.textContent=m.to;
  if(fromLogo) fromLogo.src=m.fromLogo;
  if(toLogo) toLogo.src=m.toLogo;
...
function pickSwapToToken(token){
  if(token==='cirBTC'){
  ...
  const fromLbl=document.getElementById('fromTokenLabel');
  const curFrom=fromLbl?fromLbl.textContent:'USDC';
  // Only two live tokens (USDC, EURC) — figure out which of the 4 cycle
  // modes matches the desired pairing and jump straight to it.
  let targetMode;
  if(token==='EURC') targetMode = 0;      // USDC→EURC
  else if(token==='USDC' && curFrom==='EURC') targetMode = 1; // EURC→USDC
  else if(token==='USDC' && curFrom==='cirBTC') targetMode = 3; // cirBTC→USDC
  else targetMode = 0; // fallback: USDC→EURC
  flipSwap(targetMode); // pass mode explicitly — bypasses the increment
  closeSwapToDropdown();
}

function toggleSwapToDropdown(){
  document.getElementById('swapToToken-list')?.classList.toggle('open');
}
function closeSwapToDropdown(){
  document.getElementById('swapToToken-list')?.classList.remove('open');
}
document.addEventListener('click', function(e){
  const list = document.getElementById('swapToToken-list');
        ...
        if (mintEl) {
          ...
          mintEl.innerHTML = `
            <div style="margin-top:10px;background:#1a1a1a;border:1px solid #1a1a1a;border-radius:12px;padding:14px;">
              ...
              <div style="font-size:.72rem;color:var(--text);line-height:1.7;margin-bottom:10px;">
...
function renderQR(a){
  ...
  catch{b.innerHTML='<p style="padding:10px;font-size:.7rem;color:var(--text)">QR unavailable</p>';}
}
function copyAddr(){
  if(!userAddr)return;
...
const CIO_TABS = ['overview','swap','cashin','cashout','banks','history'];
...
async function cioLoadRates(){
  try{
    ...
    ['cioRateUsdc','cioRateUsdc2','cioRateUsdc3','cioRateUsdc4'].forEach(id => { const el = document.getElementById(id); if(el) el.querySelector('.rate').textContent = usdcText; });
    ['cioRateEurc','cioRateEurc2','cioRateEurc3','cioRateEurc4'].forEach(id => { const el = document.getElementById(id); if(el) el.querySelector('.rate').textContent = eurcText; });
...
async function cioLoadBanksForSwap(){
  if(!userAddr) return;
  const sel = document.getElementById('cioSwapBank');
  const r = await fetch(CASHIO_API, {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'list-bank-accounts', walletAddress:userAddr})});
  const d = await r.json();
  if(!d.success || !d.accounts.length){ sel.innerHTML = '<option value="">No saved bank account, add one first</option>'; return; }
  sel.innerHTML = d.accounts.map(a => `<option value="${a.id}">${a.bankName}, ${a.accountNumber}</option>`).join('');
...
function cioUpdateSwapPreview(){
  ...
  const receiveEl = document.getElementById('cioSwapReceive');
  if(!rateEl || !estEl) return;
  rateEl.textContent = rate ? '1 '+token+' equals '+rate.toFixed(2)+' NGN' : 'Unavailable';
  const total = rate && amt ? (amt*rate).toFixed(2) : '0.00';
  estEl.textContent = total+' NGN';
  if(receiveEl) receiveEl.value = total;
}
async function cioSwapCreate(){
  if(!userAddr){ toast('Connect your wallet first','error'); return; }
  const amt = parseFloat(document.getElementById('cioSwapAmount').value);
  const token = document.getElementById('cioSwapToken').value;
  const bankAccountId = document.getElementById('cioSwapBank').value;
  const statusEl = document.getElementById('cioSwapStatus');
  if(!amt || amt <= 0){ statusEl.textContent = 'Enter an amount to swap.'; return; }
  if(!bankAccountId){ statusEl.textContent = 'Add and select a bank account first.'; return; }
  statusEl.textContent = 'Creating request';
  const r = await fetch(CASHIO_API, {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'create-cashout-request', walletAddress:userAddr, stablecoinAmount:amt, fromToken:token, bankAccountId})});
  const d = await r.json();
  if(!d.success){ statusEl.textContent = 'Error, '+d.error; return; }
  statusEl.textContent = 'Swap request created, pending settlement.';
  toast('Swap to Naira request created', 'success', 3000);
}
...
async function cioDownloadReceipt(transactionId){
  ...
  const rows = tx.type === 'cashin'
    ...
    : [['Transaction ID', tx.id], ['Type', 'Withdrawal'], ['Amount sent', tx.stablecoinAmount.toFixed(2)+' '+tx.fromToken], ['Exchange rate', tx.rate ? tx.rate.toFixed(2) : 'Unavailable'], ['Amount received', tx.estimatedNaira ? tx.estimatedNaira.toFixed(2)+' NGN' : 'Unavailable'], ['Bank details', tx.bankName+', '+tx.accountNumber], ['Status', tx.status]];
  rows.forEach((row, i) => {
    const y = 108 + i*30;
    ctx.fillStyle = '#a3a3a3'; ctx.font = '12px sans-serif'; ctx.textAlign = 'left';
    ctx.fillText(row[0], 28, y);
    ctx.fillStyle = '#EEF2FF'; ctx.font = 'bold 12px monospace'; ctx.textAlign = 'right';
...
function executeAgentAction(action){
  switch(action.action){
    ...
    case 'agent-pay':
      ...
      fetch(AGENT_API,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'pay-service',userAddress:userAddr,address:agentWalletAddr,url:action.serviceUrl||action.url,amount:String(action.amount||'0.01'),chain:'ARC-TESTNET'})})
...
async function adminSeedPool(){
  ...
  try{
    ...
    statusEl.innerHTML='<span style="color:#60A5FA;">Approving…</span>';
    ...
    statusEl.innerHTML='<span style="color:#60A5FA;">Adding liquidity…</span>';
    const liqTx = await swapC.addLiquidity(seedU,seedE,arcGasOpts());
    await liqTx.wait(1);
  ...
  try{
    ...
    resultsEl.innerHTML = `<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:10px;">${
      services.map((s,i) => {
        ...
        <div style="margin-bottom:8px;">${netBadge}</div>
...
function agentPageRefresh() {
  ...
  s('agentLoginSection',     !connected);
  s('agentOtpSection',       false);
  s('agentPendingSection',   false);
  s('agentConnectedSection', connected);

  const status = document.getElementById('agentWalletStatus');
  const badge  = document.getElementById('agentWalletBadge');
  const addr   = document.getElementById('agentArcAddr');
...
async function agentLoginInit() {
  ...
  try {
    ...
    const d = await r.json();
    if (d.installing) {
      agentShowResult('CLI starting up (~60s). Please try again shortly.');
      if (btn) { btn.textContent = 'Connect Agent Wallet'; btn.disabled = false; }
      return;
    }
    if (d.alreadyLoggedIn) { await agentLoadWallets(); return; }
    if (d.requestId) {
...
async function agentVerifyOtp() {
  ...
  if (!otp) { agentShowResult('Please enter the code from your email'); return; }
  const os = document.getElementById('agentOtpSection');
  const ps = document.getElementById('agentPendingSection');
  if (os) os.style.display = 'none';
  if (ps) ps.style.display = '';
  try {
    await fetch(AGENT_API, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'login-complete', requestId: agentRequestId, otp, email: agentWalletEmail }) });
    let attempts = 0;
  ...
  } catch (e) { agentResetLogin(); agentShowResult('Network error — please try again'); }
}

function agentResetLogin() {
  clearInterval(agentPollTimer);
  ['agentPendingSection','agentOtpSection'].forEach(id => { const el = document.getElementById(id); if (el) el.style.display = 'none'; });
  const ls = document.getElementById('agentLoginSection');
...
async function agentLoadWallets() {
  try {
    ...
    if (d.wallets?.['ARC-TESTNET']) {
      agentWalletAddr = d.wallets['ARC-TESTNET'];
      localStorage.setItem('nan_agent_addr', agentWalletAddr);
      agentPageRefresh();
    }
  } catch {}
